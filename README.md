...
This pipeline is responsible for the complex ETL (Extract, Transform, Load) process that turns dozens of raw files into the final, clean dataset that powers the app. This is now a two-stage process for clarity and academic robustness.
...
* build_area_codes.py: Creates a master lookup file (area_codes.csv) that maps every LSOA (2021) to its corresponding Ward (2025) and LAD (2025).

#### Stage 1: Indicator Processing (process_*.py):
...
* Its sole responsibility is to extract, clean, and geocode the raw data into a "sparse" annual parquet file.

* No temporal imputation (e.g., ffill, bfill) happens at this stage. The output files contain data only for the years it actually exists, with NaN (Not a Number) for all other years.

* Nearest Neighbour Analysis: For services like GPs, schools, and childcare, these scripts use a k-d tree spatial index (via scipy.spatial.cKDTree) to efficiently calculate the 3-nearest services for every LSOA centroid and retrieve their distances.

#### Stage 2: Master File Generation & Imputation (build_master_timeseries.py):

* This is the final and most important script, acting as the project's "Imputation Engine."

* It builds a complete 2018-2025 grid for all LSOAs.

* It merges all the 'sparse' data files from Stage 1.

* It centrally manages all data imputation logic (e.g., Last Observation Carried Forward, Next Observation Carried Backward) to fill all NaN gaps and create the final, complete lsoa_annual_indicators.parquet file.

* This separation makes it easy to test different imputation strategies (e.g., linear interpolation vs. mean-filling) by simply creating a copy of this file.

### 2. App Scoring & Aggregation (The app/ Directory)

The Streamlit app reads the final lsoa_annual_indicators.parquet file and performs all scoring and visualization in memory, cached via @st.cache_data.
...
The processed data files required by the app are not included in this repository. You must run the data processing pipeline to generate them.
...
This project includes a shell script (rebuild_data.sh) that automates the entire data build process. From your terminal in the project root:

```bash
# Make the script executable (only need to do this once)
chmod +x rebuild_data.sh

# Run the complete rebuild
bash rebuild_data.sh
```

This script will automatically:

* Delete the old data/processed directory.

* Run all the base geography scripts (e.g., build_boundaries_sw.py).

* Run all the indicator processing scripts (e.g., process_crime_data.py) to create the sparse files.

* Run the final build_master_timeseries.py to create the fully imputed master file.
