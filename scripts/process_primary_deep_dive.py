...
print("Starting PRIMARY School Deep Dive Data Processing...")
...
OUTPUT_FILE = PROCESSED_DATA_DIR / "lsoa_annual_primary_deepdive.parquet"
NEAREST_N = 3
...
def process_nearest_schools_deepdive(lsoa_gdf, school_gdf, perf_df):
    ...
    # Create lookup for static school info (name)
    school_info = school_gdf.set_index('urn')[['name']].to_dict('index')
    ...
    for i, lsoa_code in enumerate(lsoa_codes):
        ...
        school_dists = dists[i]
        # Determine the URNs of these 3 schools
        schools_for_lsoa = []
        for rank, idx in enumerate(school_indices):
            urn = school_gdf.iloc[idx]['urn']
            dist = school_dists[rank] / 1000.0  # km
            name = school_info.get(urn, {}).get('name', 'Unknown')
            schools_for_lsoa.append({'rank': rank + 1, 'urn': urn, 'dist': dist, 'name': name})
        ...
        for year in years:
            ...
            for school in schools_for_lsoa:
                ...
                urn = school['urn']
                # Base Info
                row[f'primary_school_{r}_name'] = school['name']
                row[f'primary_school_{r}_urn'] = urn
                row[f'primary_school_{r}_distance_km'] = school['dist']
                # Performance Data for this specific year
                perf_data = perf_lookup.get((urn, year), {})
                row[f'primary_school_{r}_pass_rate'] = perf_data.get('pass_rate', np.nan)
                row[f'primary_school_{r}_read_score'] = perf_data.get('read_score', np.nan)
                row[f'primary_school_{r}_math_score'] = perf_data.get('math_score', np.nan)
                row[f'primary_school_{r}_data_year'] = year if perf_data else np.nan
            records.append(row)
