...
# Output Directory for Analysis Images (Heatmaps)
OUTPUT_BASE_DIR = PROJECT_DIR / "data" / "analysis_results_2024_final_check"
OUTPUT_BASE_DIR.mkdir(parents=True, exist_ok=True)

# Output Directory for Final Data (Processed)
...
# Final Output Filename -> Goes to 'processed' directory
FINAL_OUTPUT_FILE = PROCESSED_DATA_DIR / f"lsoa_final_composite_scores_{TARGET_YEAR}.parquet"
...
# --- OECD Weighting Calculation Function (NO CHANGE) ---
...
def plot_heatmap(loadings_df, year, output_dir):
    ...
    # Use the pillar names for a clearer visual label
    rc_labels = [PILLAR_NAMES.get(int(col[2:]), col) for col in loadings_df.columns]

    # Sort the indicators by the factor they load highest on (for better visual grouping)
    ...
    sns.heatmap(
        sorted_loadings,
        annot=True,  # Annotate the heatmap cells with the data value
        fmt=".3f",  # Format the annotation to 3 decimal places
        cmap="vlag",  # Diverging color palette (e.g., for +/- values)
        cbar_kws={'label': 'Factor Loading'},  # Label the color bar
        linewidths=.5,  # Add lines between cells
        linecolor='black',
        vmin=-1,  # Set min/max for color scale to -1 and 1
        vmax=1,
        yticklabels=True,  # Show indicator variable names
        xticklabels=rc_labels  # Use the more descriptive RC names
    )

    plt.title(f'Rotated Factor Loadings Heatmap (Varimax) - {year}', fontsize=14)
...
# --- NEW: Calculation and Saving of Final Scores ---

def calculate_and_save_final_scores(df_original, weights_results, output_path):
    """
    Applies the two-level weighting formula to calculate Pillar scores and Final CI.
    Formula: CI_i = Sum_j [ w_factor_j * ( Sum_k (w_k,j * I_k,i) ) ]
    """
    print("\n" + "=" * 70)
    ...
    indicator_weights = weights_results['indicator_weights_df']
    factor_weights = weights_results['factor_weights_df']

    # We will store the names of the new pillar columns here
    pillar_columns = []

    # 2. Level 1 Aggregation: Calculate Pillar Scores (RC1, RC2, etc.)
    # Formula: Pillar_Score_j = Sum( w_k,j * Indicator_Score_k )

    for rc_name in factor_weights.index:  # Iterate through RC1, RC2, ...
        # Find indicators belonging to this RC
        rc_indicators = indicator_weights[indicator_weights['Assigned_RC'] == rc_name]

        if rc_indicators.empty:
            print(f"Warning: No indicators found for {rc_name}")
            continue

        # Initialize pillar column
        pillar_col = f"{rc_name}_Score"
        final_df[pillar_col] = 0.0

        print(f"Calculating {pillar_col}...")

        # Weighted sum of indicators
        for indicator_name, row in rc_indicators.iterrows():
            weight = row['Weight']
            if indicator_name in final_df.columns:
                # Add weighted score
                final_df[pillar_col] += final_df[indicator_name] * weight
                print(f"  + {indicator_name} (w={weight:.4f})")
            else:
                print(f"  ! Missing data column: {indicator_name}")

        pillar_columns.append(pillar_col)

    # 3. Level 2 Aggregation: Calculate Final Composite Indicator
    # Formula: CI = Sum( w_factor_j * Pillar_Score_j )

    final_df['Final_CI_Score'] = 0.0
    print("\nCalculating Final_CI_Score...")

    for rc_name in factor_weights.index:
        pillar_col = f"{rc_name}_Score"
        if pillar_col in final_df.columns:
            f_weight = factor_weights.loc[rc_name, 'Factor_Weight']

            final_df['Final_CI_Score'] += final_df[pillar_col] * f_weight
            print(f"  + {pillar_col} (Factor Weight={f_weight:.4f})")

    # 4. Rename Columns for Clarity (RC1_Score -> Socio-Economic Score)
    # Mapping based on PILLAR_NAMES constant
    rename_map = {}
    for i in range(1, N_COMPONENTS + 1):
        rc_key = f"RC{i}_Score"
        # Extract meaningful name (e.g., 'Socio-Economic Deprivation')
        clean_name = PILLAR_NAMES[i].split(': ')[1].replace(' ', '_') + "_Score"
        rename_map[rc_key] = clean_name

    final_df = final_df.rename(columns=rename_map)

    # 5. Clean up and Save
    # Keep only identification, original variables, pillar scores, and final score
    cols_to_keep = ['year', 'Final_CI_Score'] + list(rename_map.values()) + list(indicator_weights.index)

    # If area_code is in index, reset it so it saves as a column
    if final_df.index.name == 'area_code':
        final_df = final_df.reset_index()
        cols_to_keep.insert(0, 'area_code')
    ...
    # Save to Parquet
    print(f"\nSaving final scored data to: {output_path}")
    output_df.to_parquet(output_path, index=False)

    print("\nSample Output (First 5 rows):")
    print(output_df[['area_code', 'Final_CI_Score']].head().to_string(index=False))
...
def main():
    ...
    # Rotated Loadings Matrix (Full Precision) - NOW INCLUDED FOR CHECKING
    print("\n\n1.2 Rotated Loadings Matrix (A_rotated) - Component Loadings:")

    # Use to_string() with the global pandas setting for 5 decimal places
    ...
    # --- B. OECD WEIGHTING PROCESS (3-Step) ---
    ...
    calculate_and_save_final_scores(df_year, weights_results, FINAL_OUTPUT_FILE)

    print("\n" + "=" * 70)
    print("Analysis Complete. Results printed to terminal, heatmap saved, and final scores saved to processed data.")
    print("=" * 70)
