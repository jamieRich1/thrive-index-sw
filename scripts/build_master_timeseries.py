...
print("Starting master annual indicator table build (FINAL FIX)...")
...
# --- INPUT FILES (Sparse Annual Data - Now Wide Format) ---
...
def load_and_merge_file(master_df, file_path, on_cols, file_desc, cols_to_drop=None):
    if file_path.exists():
        ...
        # Deduplicate to prevent row explosion
        df_to_merge = df_to_merge.drop_duplicates(subset=on_cols)

        if cols_to_drop:
            # Drop columns if they exist in the incoming dataframe
...
def generate_impute_list(master_df):
    ...
    # 2. Dynamic "Wide" Columns (Schools, GP, Childcare)
    # We want to impute scores/metrics, but NOT names or IDs.
    ...
    for col in master_df.columns:
        # Check if column starts with a valid prefix AND ends with a valid metric
        if any(col.startswith(p) for p in prefixes) and any(col.endswith(m) for m in metrics):
            cols.append(col)

    # Remove duplicates and filter to existing numeric columns
    valid_cols = sorted(list(set([c for c in cols if c in master_df.columns])))
    # Ensure we only impute numeric types
...
def main():
    try:
        ...
        # 2. Merge Sparse Data (Now Wide Format)
        # These files now contain EVERYTHING: names, distances, scores.
        ...
        # 3. Merge Static Data (Only the truly static stuff)
        ...
        # --- REMOVED REDUNDANT STATIC MERGES HERE TO FIX _X/_Y ERRORS ---
        # We do NOT merge STATIC_SECONDARY_EDU_FILE etc. because that data is already in Step 2.
        ...
        # A. CALCULATE AGGREGATES (Collapsing Wide Format back to Averages for the App)
        # Note: We take row-wise means across the 3 schools/GPs
        ...
        # B. RECALCULATE AIR QUALITY SCORE
